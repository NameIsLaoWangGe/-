...
export class _GameEffects2D {
    ...
    static _interfacePointJet(): void {
        ...
        LwgTimer.frameNumLoop(5, 15, this, () => {
            for (let index = 0; index < 5; index++) {
                LwgEff2D.Particle.downwardSpray(Laya.stage, new Laya.Point(Laya.stage.width * 1 / 4, -diff), [10, 30], [10, 30], [0, 90], [LwgEff2D.SkinUrl.矩形1], null,);
                LwgEff2D.Particle.downwardSpray(Laya.stage, new Laya.Point(Laya.stage.width * 3 / 4, -diff), [10, 30], [10, 30], [90, 180], [LwgEff2D.SkinUrl.矩形1], null,);
                LwgEff2D.Particle.downwardSpray(Laya.stage, new Laya.Point(-diff, Laya.stage.height / 4), [10, 30], [10, 30], [0, 90], [LwgEff2D.SkinUrl.矩形1], null);
                LwgEff2D.Particle.downwardSpray(Laya.stage, new Laya.Point(Laya.stage.width + diff, Laya.stage.height / 4), [10, 30], [10, 30], [90, 180], [LwgEff2D.SkinUrl.矩形1], null,);
            }
        })
    }
    ...
    static _completeSidelingCross(): void {
        ...
        Img.height = _height;
        Img.rotation = 40;
        LwgTools.Node.changePivot(Img, 0, _height / 2);
        Img.pos(0, 0);
    ...
    static _oneFireworks(point: Laya.Point): void {
        ...
        const dis = LwgTools.Num.randomOneInt(100, 300);
        for (let index = 0; index < count * 2; index++) {
            LwgEff2D.Particle.sprayRound(Laya.stage, point, null, [20, 40], null, [LwgEff2D.SkinUrl.花4], null, [dis, dis], [time, time]);
        }
        for (let index = 0; index < count * 2; index++) {
            LwgEff2D.Particle.sprayRound(Laya.stage, point, null, [20, 40], null, [LwgEff2D.SkinUrl.花4], null, [50, dis - 20], [time, time]);
        }
    }
    ...
    static _bothBlinkOnSprite(scene: Laya.Scene, sprite: Laya.Sprite): any {
        ...
        LwgTimer.frameRandomLoop(30, 50, caller, () => {
            LwgEff2D.Glitter.blinkStar(scene, new Laya.Point(sprite.x - 350, sprite.y), [150, 100], [LwgEff2D.SkinUrl.星星1], null, [80, 80]);
        }, true)
        // 星星闪烁动画右边
        LwgTimer.frameRandomLoop(30, 50, caller, () => {
            LwgEff2D.Glitter.blinkStar(scene, new Laya.Point(sprite.x + 350, sprite.y), [150, 100], [LwgEff2D.SkinUrl.星星1], null, [80, 80]);
