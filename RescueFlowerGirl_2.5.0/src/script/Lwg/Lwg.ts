...
/**游戏控制*/
export module LwgGame {
    /**游戏开关,控制步骤，不控制暂停，以现在的机制，一般用不上*/
    export let onOff = true;
    /**游戏暂停*/
    export const pause = {
        get value(): boolean {
            return onOff;
...
export module LwgScene {
    ...
    export function aniFlow(openScene: Laya.Scene, closeScene: Laya.Scene,/** _openFunc: Function, _closeFunc: Function*/): void {
        ...
        if (closeScene) {
            ...
            // 2.检查关闭动画内部是否被重写了
            closeScript = closeScene[closeScene.name] as SceneBase;
            if (closeScript) {
    ...
    export function openScene(openName: string, openData?: object, closeName?: string, returnBtn?: boolean, /*openfunc?: Function, zOrder?: number*/): void {
        ...
        Laya.Scene.load('Scene/' + openName + '.json', Laya.Handler.create(this, function (scene: Laya.Scene) {
            ...
            let openScene = LwgTools.Node.checkChildren(Laya.stage, openName) as Laya.Scene;
            if (openScene) {
                openScene.close();
    ...
    export class ScriptBase extends Laya.Script {
        ...
        /**全局查找*/
        _FindImg(name: string,): LwgNode.Image {
            return this.getFind(name, '_FindImg');
        }
        /**全局查找*/
        _FindSp(name: string,): LwgNode.Sprite {
        ...
        /**存储一个数字*/
        _storeNum(name: string, _func?: Function, initial?: number): LwgStorage.NumVariable {
            return LwgStorage.num(`${this.ownerSceneName}/${name}`, _func, initial);
        }
        /**存储一个字符串*/
        _storeStr(name: string, _func?: Function, initial?: string): LwgStorage.StrVariable {
            return LwgStorage.str(`${this.ownerSceneName}/${name}`, _func, initial);
        }
        /**存储一个布尔值*/
        _storeBool(name: string, _func?: Function, initial?: boolean): LwgStorage.BoolVariable {
            return LwgStorage.bool(`${this.ownerSceneName}/${name}`, _func, initial);
        }
        /**存储一个布尔值*/
        _storeArray(name: string, _func?: Function, initial?: Array<any>): LwgStorage.ArrayVariable {
        ...
        */
        _btnDown(target: Laya.Node, down?: Function, effect?: string): void {
            LwgClick.on(effect == undefined ? LwgClick.Use.value : effect, target, this, (e: Laya.Event) => {
        ...
        _btnMove(target: Laya.Node, move: Function, effect?: string): void {
            LwgClick.on(effect == undefined ? LwgClick.Use.value : effect, target, this, null, (e: Laya.Event) => {
       ...
       */
        _btnUp(target: Laya.Node, up: Function, effect?: string): void {
            LwgClick.on(effect == undefined ? LwgClick.Use.value : effect, target, this, null, null, (e: Laya.Event) => {
        ...
        _btnOut(target: Laya.Node, out: Function, effect?: string): void {
            LwgClick.on(effect == undefined ? LwgClick.Use.value : effect, target, this, null, null, null, (e: Laya.Event) => {
        ...
        _btnFour(target: Laya.Node, down?: Function, move?: Function, up?: Function, out?: Function, effect?: string): void {
            LwgClick.on(effect == null ? effect : LwgClick.Use.value, target, this,
                (e: Laya.Event) => {
                    this.checkBtnClick(target, down, e);
                },
                (e: Laya.Event) => {
        ...
        lwgOnDisable(): void { };
        onStageMouseDown(e: Laya.Event): void { LwgClick.checkStage() && this.lwgOnStageDown(e) };
        onStageMouseMove(e: Laya.Event): void { LwgClick.checkStage() && this.lwgOnStageMove(e) };
        onStageMouseUp(e: Laya.Event): void { LwgClick.checkStage() && this.lwgOnStageUp(e) };
    ...
    export class SceneBase extends ScriptBase {
        ...
        /**常用动画组件获取*/
        _SkVar(name: string): Laya.Skeleton {
        ...
        _ImgVar(name: string): LwgNode.Image {
    ...
    export class ObjectBase extends ScriptBase {
        ...
        _lwgDestroyAndClear(): void {
            this._Owner.destroy();
            this.clear();
        }
        /**挂载当前脚本的节点*/
        get _Owner(): LwgNode.Sprite {
        ...
        _SceneImg(name: string): LwgNode.Image {
        ...
        getChild(name: string, type: string): any {
            if (!this[`${type}${name}`]) {
                const child = this._Owner.getChildByName(name) as Laya.Sprite;
                if (child) {
        ...
        _FontClipChild(name: string): Laya.FontClip {
            return this.getChild(name, '_TapFontClip');
        }
        _SKChild(name: string): Laya.FontClip {
            return this.getChild(name, '_TapFontClip');
        }
        onAwake(): void {
...
export module LwgNode {
    export type BaseProperty = {
         ...
         * @param OtherNode 节点
         */
        disByNode: (OtherNode: Laya.Sprite) => number;
        /**
         * 和另一个坐标之间的距离,用世界坐标进行计算
         * @param point 节点
         */
        globleDisByPoint: (point: Laya.Point) => number;
        /**
        * 和另一个坐标之间的距离
        * @param point 节点
        */
        localDisByPoint: (point: Laya.Point) => number;
        /**
          * 和另一个坐标之间的距离,用世界坐标进行计算
          * @param point 节点
         */
        childGPoint: (Child: Laya.Sprite) => Laya.Point;
    }
    ...
    export function _addProperty(Node: Laya.Sprite, nodeType?: string): void {
        if (!Node) return;
        let _proType: BaseProperty;
        switch (nodeType) {
            case 'Img':
                _proType as ImageProperty;
                break;
            case 'box':
                _proType as BoxProperty;
                break;

            default:
                _proType as BaseProperty;
                break;
        }
...
export module LwgDialogue {
    enum Skin {
        blackBord = 'Lwg/UI/rectangle_mask_07.png'
    }
    /**
     * 创建一个提示框
    ...
    export function getDialogContent(useWhere: string, name: number): Array<string> {
        let dia;
        for (let index = 0; index < dialogContent.Array.length; index++) {
            const element = dialogContent.Array[index];
    ...
    export module Gold {
        ...
        function updateCount(): void {
            ...
            if (Num['sheet']) {
                ...
                Num['text'] = LwgTools._Format.formatNumber(num.value);
            }
        }
        /**增加金币以并且在节点上也表现出来*/
        ...
        */
        export function getAni_Single(parent: Laya.Sprite, number: number, width: number, height: number, url: string, firstPoint: Laya.Point, targetPoint: Laya.Point, func1?: Function, func2?: Function): void {
         ...
         * @param func2 金币全部创建完成后的回调
         */
        export function getAni_Heap(parent?: Laya.Sprite, number?: number, width?: number, height?: number, url?: string, firstPoint?: Laya.Point, targetPoint?: Laya.Point, func1?: Function, func2?: Function): void {
            for (let index = 0; index < number; index++) {
...
export module LwgDate {
    ...
    export function init(): void {
        const d = new Date;
        loginInfo = LwgStorage.arrayArr('LwgDate/loginInfo');
        loginInfo.value.push([d.getFullYear(), d.getMonth() + 1, d.getDate(), d.getDay(), d.getHours(), d.getMinutes(), d.getSeconds()])
        let arr: Array<Array<any>> = [];
        if (loginInfo.value.length > 0) {
            for (let index = 0; index < loginInfo.value.length; index++) {
                arr.push(loginInfo.value[index]);
            }
        }
    ...
    export let loginInfo: LwgStorage.ArrayArrVariable;
    /**玩家登录的总次数*/
    export let loginCount: LwgStorage.NumVariable;
    /**今天登陆了几次*/
    export const loginToday = {
        get num(): number {
    ...
    export const last = {
        get date(): number {
            if (loginInfo.value.length > 1) {
                return loginInfo.value[loginInfo.value.length - 2][2];
            } else {
...
export module LwgTimer {
     ...
     * @param coverBefore 是否覆盖之前的延迟执行，默认为 true 。
     */
    export function frameNumLoop(delay: number, num: number, caller: any, method: Function, compeletMethod?: Function, immediately?: boolean, args?: any[], coverBefore?: boolean): void {
        if (immediately) {
            if (onOff) {
                method();
            }
     ...
     * @param coverBefore 是否覆盖之前的延迟执行，默认为 true 。
     */
    export function numRandomLoop(delay1: number, delay2: number, num: number, caller: any, method: Function, compeletMethod?: Function, immediately?: boolean, args?: any[], coverBefore?: boolean): void {
     ...
     * @param coverBefore 是否覆盖之前的延迟执行，默认为 true 。
     */
    export function frameNumRandomLoop(delay1: number, delay2: number, num: number, caller: any, method: Function, compeletMethod?: Function, immediately?: boolean, args?: any[], coverBefore?: boolean): void {
        immediately && onOff && method();
        let num0 = 0;
        var func = () => {
            let delay = LwgTools.Num.randomOneInt(delay1, delay2);
     ...
     * @param coverBefore 是否覆盖之前的延迟执行，默认为 true 。
     */
    export function numLoop(delay: number, num: number, caller: any, method: Function, compeletMethod?: Function, immediately?: boolean, args?: any[], coverBefore?: boolean): void {
        if (immediately) {
            method();
        }
        let num0 = 0;
    ...
    */
    ...
    /**
     * @export 清理对象上的所有计时器
     * @param {any} caller 清理的数组
     */
    export function clearAll(caller: any): void {
        Laya.timer.clearAll(caller);
    }
    /**
     * @export 清理对象上的所有计时器上的函数
     * @param  {any} caller 清理的数组
     * @param  {Function} func 清理的数组
     */
    export function _clear(caller: any, func: Function): void {
        Laya.timer.clear(caller, func);
    }
}

/**节点的行为*/
export module ActionAdmin {
...
export module LwgSceneAni {
    ...
    export module Shutters {
        ...
        /**延时，有时候绘制需要时间*/
        let commonDelay = 150;
        function moveClose(sp: Laya.Sprite, tex: Laya.Texture, scaleX: number, scealeY: number): void {
        ...
        function moveOpen(sp: Laya.Sprite, tex: Laya.Texture, scaleX: number, scealeY: number): void {
        ...
        function moveRule(sp: Laya.Sprite, tex: Laya.Texture, scaleModul: string, open: boolean): void {
            if (open) {
                if (scaleModul === 'x') {
                    moveOpen(sp, tex, 0, 1);
                } else {
        ...
        /**
          ...
          * @memberof _shutters
           */
        function createNoMaskSp(x: number, y: number, width: number, height: number, tex: Laya.Texture, scaleModul: string, open: boolean): Laya.Sprite {
        ...
        /**
          ...
          * @memberof _shutters
          */
        function createMaskSp(Scene: Laya.Scene, scaleModul: string, open: boolean): Laya.Sprite {
            const sp = new Laya.Sprite;
            Laya.stage.addChild(sp);
         ...
         * @param  sp 所在节点
         * */
        function createMask(sp: Laya.Sprite): Laya.Image {
        ...
        /**横向*/
        export function crosswise(Scene: Laya.Scene, open: boolean): void {
        ...
        /** 竖向*/
        export function vertical(Scene: Laya.Scene, open?: boolean): void {
            for (let index = 0; index < commonNum; index++) {
        ...
        /**横竖交叉*/
        export function croAndVer(Scene: Laya.Scene, open: boolean): void {
        ...
        /**左斜缩放*/
        export function rSideling(Scene: Laya.Scene, open: boolean): void {
            for (let index = 0; index < commonNum; index++) {
                let addLen = 1000;
        ...
        /**右斜缩放*/
        export function lSideling(Scene: Laya.Scene, open: boolean): void {
            for (let index = 0; index < commonNum; index++) {
                const sp = createMaskSp(Scene, 'x', open);
                const Mask = createMask(sp);
         ...
         * @memberof _shutters
         */
        export function sidelingIntersection(Scene: Laya.Scene, open: boolean): void {
            for (let index = 0; index < commonNum; index++) {
                let addLen = 1000;
        ...
        /**横竖三个随机播放*/
        export function randomCroAndVer(Scene: Laya.Scene, open: boolean): void {
        ...
        /**随机播放*/
        export function random(Scene: Laya.Scene, open: boolean): void {
        ...
        export class Open {
            ...
            static _paly(type: string, Scene: Laya.Scene): number {
                ...
                return closeAniTime = commonTime + commonDelay;
            };
        }
    }

    /**单个附加*/
    export module SickIn {
        function stickIn(Scene: Laya.Scene, type: string): number {
            let sumDelay: number = 0;
            let time: number = 700;
            let delay: number = 100;
            if (Scene.getChildByName('Background')) {
                LwgAni2D.fadeOut(Scene.getChildByName('Background') as Laya.Sprite, 0, 1, time);
            }
            ...
            for (let index = 0; index < stickInLeftArr.length; index++) {
                ...
                if (element.name !== 'Background' && element.name.substr(0, 5) !== 'NoAni') {
                    ...
                    //         LwgTools.Node.changePivot(element, 0, 0);

                    //         break;
                    //     case _Type.stickIn.upRightDownLeft:
                    //         element.rotation = element.y > Laya.stage.height / 2 ? -180 : 180;
                    //         LwgTools.Node.changePivot(element, element.rotation == 180 ? element.width : 0, 0);

                    //         break;
                    //     case _Type.stickIn.randomstickIn:
                    //         element.rotation =  LwgTools.Num.randomOneHalf() == 1 ? 180 : -180;
                    //         LwgTools.Node.changePivot(element,  LwgTools.Num.randomOneHalf() == 1 ? 0 : element.width,  LwgTools.Num.randomOneHalf() == 1 ? 0 : element.height);
                    //         break;
                    //     default:
                    //         break;
                    // }
...
export module LwgStorage {
    ...
    export class NumVariable extends admin {
        value: number;
    }
    export class StrVariable extends admin {
        value: string;
    }
    export class BoolVariable extends admin {
        value: boolean;
    }
    export class ArrayVariable extends admin {
        value: Array<any>;
    }
    export class ArrayArrVariable extends admin {
    ...
    * */
    export function num(name: string, _func?: Function, initial?: number): NumVariable {
        if (!this[`_num${name}`]) {
            const obj: NumVariable = {
                get value(): number {
     ...
     * */
    export function str(name: string, _func?: Function, initial?: string): StrVariable {
        if (!this[`_str${name}`]) {
            this[`_str${name}`] = {
                get value(): string {
                    if (Laya.LocalStorage.getItem(name)) {
     ...
     * */
    export function bool(name: string, _func?: Function, initial?: boolean): BoolVariable {
        if (!this[`_bool${name}`]) {
            this[`_bool${name}`] = {
                get value(): any {
                    if (Laya.LocalStorage.getItem(name)) {
    ...
    * */
    export function array(name: string, _func?: Function, initial?: Array<any>): ArrayVariable {
        if (!this[`_array${name}`]) {
            this[`_array${name}`] = {
                get value(): Array<any> {
                    try {
    ...
    /**
      ...
      * @param initial 初始值，如果有值了则无效，默认为[]
      * */
    export function obj(name: string, _func?: Function, initial?: {}): _Object {
        if (!this[`_obj${name}`]) {
            this[`_obj${name}`] = {
                get value(): {} {
                    try {
     ...
     * @param initial 初始值，如果有值了则无效，默认为[]
     * */
    export function arrayArr(name: string, _func?: Function, initial?: Array<Array<any>>): ArrayArrVariable {
...
export module LwgData {
    ...
    /**获取路径*/
    export let _unlockWayType: BaseUnlockWay = {
        ads: 'ads',
        gold: 'gold',
        diamond: 'diamond',
        customs: 'customs',
    ...
    export class Item extends LwgScene.ObjectBase {
        ...
        /**当对list进行一些特殊操作的时候，可能会导致事件注册不上，那么这个变量会在$render中进行注册*/
        buttonOn = false;
        /**渲染内容*/
        $render(): void { };
        /**事件注册*/
        $button(): void { };
        /**onStart替代了lwgOnStart,不用onawake，有时候初始化不进去*/
        $onStart(): void { };
        lwgOnStart(): void {
            this.$onStart();
            if (!this.buttonOn) {
    ...
    export class Table {
         ...
         * 进度条的滑动开关
         */
        get _ListScrollBarTouchScrollEnable(): boolean {
            return this._List.scrollBar.touchScrollEnable;
        };
        set _ListScrollBarTouchScrollEnable(bool: boolean) {
            this._List.scrollBar.touchScrollEnable = bool;
        }

        /**上个版本的表格*/
        _lastArr: BaseProperty[] = [];
        /**是否启用本地存储*/
        ...
        set _List(list: Laya.List) {
            ...
            list.renderHandler = new Laya.Handler(this, (cell: Laya.Box, index: number) => {
                if (this._listRenderScript) {
                    let _item: Item = cell.getComponent(this._listRenderScript);
                    if (!_item) {
         ...
         * @param addCompare 是否对比原来的，不对比则先获取，以后操作便会替换。
         * @param lastVtableName 如果表格发生改变，对比上个版本的数据表将一些成果继承赋值，默认属性为check，compelet，degreeNum，getAward。
         * @param lastProArr 指定上个版本需要继承的属性，指定后默认完成的属性不会自动继承。
         */
        constructor(tableName?: string, _tableArr: Array<any> = null, localStorage: boolean = true, addCompare: boolean = true, lastVtableName?: string, lastProArr?: string[]) {
        ...
        */
        ...
        /**
         * 把数组中已经完成的对象中的某个属性值累加起来，一般用在整个表属于一个对象的情况，例如总血量，总攻击力
         * @param {string} proName 属性名
         */
        _sumProByCompelet(proName: string): number {
            let sum = 0;
            for (let index = 0; index < this._arr.length; index++) {
                const element = this._arr[index];
        ...
        /**
         * 把数组对象中的某个属性值累加起来，无论完成不完成，一般用在整个表属于一个对象的情况，例如总血量，总攻击力
         * @param {string} proName 属性名
         * @return {*}  {number}
         */
        _sumProByAll(proName: string): number {
            let sum = 0;
            for (let index = 0; index < this._arr.length; index++) {
                const element = this._arr[index];
                if (element[proName]) {
        ...
        /**
          ...
          * */
        _getPitchIndexInListArr(): number {
            if (this._List) {
         ...
         * @param func 结束回调
         * */
        _listTweenToDiffIndexByPitch(diffIndex: number, time: number, func?: Function): void {
            const index = this._getPitchIndexInListArr();
        ...
        */
        ...
        /**
         * 通过某个属性名获取第一个复合这个属性值的对象
         * @param {string} proName 属性名
         * @param {*} value 属性值
         */
        _getFirstObjByPro(proName: string, value: any): BaseProperty {
            for (const key in this._arr) {
                if (Object.prototype.hasOwnProperty.call(this._arr, key)) {
                    const element = this._arr[key];
         ...
         * @param name 名字
        */
        _getObjByName(name: string): BaseProperty {
            for (const key in this._arr) {
                if (Object.prototype.hasOwnProperty.call(this._arr, key)) {
                    const element = this._arr[key];
                    if (element.name == name) {
         ...
         * @param level 等级
        */
        _getFirstObjByLevel(level: number): BaseProperty {
            for (const key in this._arr) {
                if (Object.prototype.hasOwnProperty.call(this._arr, key)) {
                    const element = this._arr[key];
         ...
         * @param level 等级
        */
        _getObjArrByLevel(level: number): BaseProperty[] {
            const arr = [];
            for (const key in this._arr) {
                if (Object.prototype.hasOwnProperty.call(this._arr, key)) {
                    const element = this._arr[key];
        ...
        * @param serial 序号
       */
        _getObjBySerial(serial: number): any {
            for (const key in this._arr) {
                if (Object.prototype.hasOwnProperty.call(this._arr, key)) {
                    const element = this._arr[key];
        ...
        /**为所有对象设置一个属性值*/
        _setOneProForAll(pro: string, value: any): void {
            for (let index = 0; index < this._arr.length; index++) {
                const element = this._arr[index];
                element[pro] = value;
            }
        ...
        /**设置一个对象完成*/
        _setCompleteByName(name: string): void {
            this._setProperty(name, this._property.complete, true);
            this._refreshAndStorage();
        }

        /**
         *通过属性名和属性值，锁定某个对象，设置象完成
         * @param {string} proName 属性名
         * @param {*} value 属性值
         * @memberof _Table
         */
        _setCompleteByProName(proName: string, value: any): void {
            for (let index = 0; index < this._arr.length; index++) {
                const element = this._arr[index];
        ...
        */
        ...
        /**
          ...
          * @param
         */
        _eachArrByCompelet(func: Function): void {
            for (let index = 0; index < this._arr.length; index++) {
                const element = this._arr[index];
                func && func(element);
            }
        ...
        /**
          ...
          * @param
         */
        _eachArrByNoCompelet(func: Function): void {
            for (let index = 0; index < this._arr.length; index++) {
                const element = this._arr[index];
                if (!element.complete) {
                    func && func(element);
        ...
        _addObj(obj: any): void {
            // 必须拷贝
            let _obj = LwgTools.ObjArray.objCopy(obj) as BaseProperty;
            for (let index = 0; index < this._arr.length; index++) {
                const element = this._arr[index];
         ...
         * */
        _addObjArr(objArr: Array<any>): void {
        ...
        /**
         * 通过名称删除数组中的某个对象,如果有好几个此名称的，则会全部删掉
         * @param {string} name
         * @memberof _Table
         */
        _deleteListObjByName(name: string): void {
            for (let index = 0; index < this._arr.length; index++) {
                const element = this._arr[index];
                if (element.name === name) {
         ...
         * @param {*} value
         */
        _deleteListObjByPro(pro: string, value: any): void {
            for (let index = 0; index < this._arr.length; index++) {
                const element = this._arr[index];
    ...
    /**
     * 获取本地存储数组和文件中数据表对比，把本地没有的数据对象添加到本地
     * @param tableArr 表格
     ...
     * @param propertyName 数组中每个对象中同一个属性名，通过这个名称进行对比
     */
    function compareAdd(tableArr: Array<any>, storageName: string, propertyName: string): Array<any> {
        // 第一步，先尝试从本地缓存获取数据，
    ...
    /**
      ...
      * @param propertyName 数组中每个对象中同一个属性名，通过这个名称进行对比
      */
    function jsonCompare(url: string, storageName: string, propertyName: string): Array<any> {
        // 第一步，先尝试从本地缓存获取数据，
...
export module LwgColor {
    ...
    * @param b
     ...
     */
    export function changeOnce(node, RGBA: Array<number>, time: number, func?: Function): void {
        if (!node) {
            return;
        }
        let cf = new Laya.ColorFilter();
    ...
    export module Particle {
        ...
        export class Caller {
            ...
            boxInit(parent: Laya.Sprite3D | Laya.Scene3D, position: [number, number, number], sectionSize: [[number, number, number], [number, number, number]], sectionRotation: [[number, number, number], [number, number, number]], texArr: Laya.Texture2D[], colorRGBA: [[number, number, number, number], [number, number, number, number]]): void {
                ...
                mat.albedoTexture = texArr ? LwgTools._Array.randomGetOne(texArr) : tex2D.圆形发光.texture2D;
...
export module LwgClick {
    ...
    export let filter = {
        get value(): string {
            return this['LwgClick/filter'] ? this['LwgClick/filter'] : filterType.all;
        },
        set value(val: string) {
            this['LwgClick/filter'] = val;
            // 自动清空
            switch (val) {
                case filterType.all || filterType.none || filterType.stage:
        ...
        /**如果_filter==someBtnIncludeStage||filter==someBtnExcludeStage时，可以放入节点名称，那么这些几点即可点击，直接加入会默认为_filterType.someBtnExcludeStage*/
        get nodeSelection(): string[] {
            return this['LwgClick/_nodeSelection'];
        },
        set nodeSelection(arr: string[]) {
            this['LwgClick/_nodeSelection'] = arr;
            switch (filter.value) {
                case filterType.someBtnIncludeStage || filterType.someBtnExcludeStage:
                    filter.value = filterType.someBtnExcludeStage;
                    break;
                default:
                    break;
            }
     ...
     * @return {*}  {boolean} 
     */
    export function checkTarget(targetName: string): boolean {
        let targetClick = false;
        if (absolute) {
            if (filter.nodeSelection.length > 0) {
                for (let index = 0; index < filter.nodeSelection.length; index++) {
                    const _name = filter.nodeSelection[index];
                    if (_name === targetName) {
                        targetClick = true;
                    }
                }
     ...
     * @return {*}  {boolean} 
     */
    export function checkStage(): boolean {
        let stageClick = false;
        if (LwgClick.absolute) {
            if (filter.value === filterType.all || filter.value === filterType.stage || filter.value === filterType.someBtnIncludeStage) {
                stageClick = true;
            }
        }
        return stageClick;
     ...
     * @return {*}  {*}
     */
    export function effectSwitch(effectType: string): any {
        let btnEffect: any;
        switch (effectType) {
            case _Type.no:
     ...
     * @param out 出屏幕函数
     */
    export function on(effect: string, target: Laya.Node, caller: any, down?: Function, move?: Function, up?: Function, out?: Function): void {
    ...
    * @param effect 效果类型 1.'largen'
     ...
     * @param out 出屏幕函数
     */
    export function off(effect: string, target: any, caller: any, down?: Function, move?: Function, up?: Function, out?: Function): void {
        const btnEffect = effectSwitch(effect);
    ...
    */
    ...
    export function leftRight_Shake(node, range, time, delayed?: number, func?: Function): void {
        ...
        Laya.Tween.to(node, { x: node.x - range }, time, null, Laya.Handler.create(this, function () {
            // LwgAudio.playSound(Enum.AudioName.commonShake, 1);
  ...
  */
    ...
    */
    ...
    export function swell_shrink(node, firstScale, scale1, time, delayed?: number, func?: Function): void {
        // LwgAudio.playSound(Enum.AudioName.commonPopup, 1);
        if (!delayed) {
            delayed = 0;
        }
...
export module LwgSet {
    ...
    */
    export function createBtnSet(x: number, y: number, width?: number, height?: number, skin?: string, parent?: Laya.Sprite, ZOder?: number): Laya.Image {
    ...
    */
    export function btnSetAppear(delayed?: number, x?: number, y?: number): void {
        ...
        if (delayed) {
            LwgAni2D.scale_Alpha(BtnSet, 0, 1, 1, 1, 1, 1, delayed, 0, f => {
                BtnSet.visible = true;
            });
        } else {
            BtnSet.visible = true;
    ...
    */
    export function btnSetVinish(delayed?: number): void {
        ...
        if (delayed) {
            LwgAni2D.scale_Alpha(BtnSet, 1, 1, 1, 1, 1, 0, delayed, 0, f => {
...
export module LwgAudio {
     ...
     */
    export function playDefeatedSound(url?: string, number?: number, func?: Function, soundVolume?: number) {
        if (LwgSet.sound.switch) {
    ...
    /**通用胜利音效播放
      ...
      * @param soundVolume 音量大小0-1，默认1
      */
    export function playVictorySound(url?: string, number?: number, func?: Function, soundVolume?: number) {
        if (LwgSet.sound.switch) {
            Laya.SoundManager.soundVolume = soundVolume ? soundVolume : 1;
...
export module LwgTools {
    ...
    * @param b
     */
    export function color_RGBtoHexString(r: number, g: number, b: number) {
        return '#' + ("00000" + (r << 16 | g << 8 | b).toString(16)).slice(-6);
    }
    /**格式化*/
...
export module LwgExecution {
    ...
    /**指代当前剩余体力节点*/
    export let ExNode: Laya.Sprite;
    /**
     * 创建通用剩余体力数量prefab
     * @param parent 父节点
     */
    export function createExecutionNode(parent: Laya.Sprite): void {
        let sp: Laya.Sprite;
        Laya.loader.load('prefab/ExecutionNum.json', Laya.Handler.create(this, function (prefab: Laya.Prefab) {
            ...
            num.value = execution.value.toString();
            sp.pos(297, 90);
            sp.zOrder = 50;
            ExNode = sp;
            ExNode.name = 'ExNode';
     ...
     * @param func 回调函数
    */
    export function addExecution(x: number, y: number, func: Function): void {
        let sp: Laya.Sprite;
        Laya.loader.load('prefab/execution.json', Laya.Handler.create(this, function (prefab: Laya.Prefab) {
            ...
            sp.zOrder = 50;
            if (ExNode) {
                LwgAni2D.move(sp, ExNode.x, ExNode.y, 800, () => {
                    LwgAni2D.fadeOut(sp, 1, 0, 200, 0, () => {
                        LwgAni2D.upDwon_Shake(ExNode, 10, 80, 0, null);
                        if (func) {
                            func();
                        }
                    });
    ...
    export function createConsumeEx(subEx: number): void {
        ...
        label.bold = true;
        label.color = '#59245c';
        label.x = ExNode.x + 100;
        label.y = ExNode.y - label.height / 2 + 4;
        label.zOrder = 100;
        LwgAni2D.fadeOut(label, 0, 1, 200, 150, () => {
            LwgAni2D.leftRight_Shake(ExNode, 15, 60, 0, null);
    ...
    export class ExecutionNode extends LwgScene.ObjectBase {
        ...
        lwgOnAwake(): void {
            ...
            // 获取上次的体力
            let d = new Date;
            if (d.getDate() !== addExDate.value) {
                execution.value = maxEx;
            } else {
                if (d.getHours() == addExHours.value) {
                    console.log(d.getMinutes(), addMinutes.value);
                    execution.value += (d.getMinutes() - addMinutes.value);
                    if (execution.value > maxEx) {
                        execution.value = maxEx;
                    }
                } else {
                    execution.value = maxEx;
            ...
            this.Num.value = execution.value.toString();
            addExDate.value = d.getDate();
            addExHours.value = d.getHours();
            addMinutes.value = d.getMinutes();
        }
